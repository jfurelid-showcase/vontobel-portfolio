...
async function main() {
  ...
  try {
    ...
    // There's no reliable "next page" indicator in the DOM (confirmed by
    // the earlier debug run — guessing at one silently stopped after page
    // 1 every time), so instead: keep requesting pages until a page's rows
    // are identical to the previous page's (a strong sign NGM is just
    // re-serving the last real page for any page number past the end) or
    // a page comes back with zero rows.
    let prevSignature = null;
    for (let p = 1; p <= MAX_PAGES; p++) {
      const { headers, rows } = await scrapePage(page, p);

      if (rows.length === 0) {
        console.log(`page ${p}: 0 rows — reached the end.`);
        break;
      }

      const signature = JSON.stringify(rows.map((r) => r.href || r.cells.join("|")));
      if (signature === prevSignature) {
        console.log(`page ${p}: identical to page ${p - 1} — reached the end, stopping.`);
        break;
      }
